...
"""
This module "ling" contains functions that manipulate and interpret strings
"""


def WordNgramStat(InputString, nMin, nMax):
    """
    Create a dictionary that counts each nGram in an input string. Delimiters are spaces.
...
def GermanToSevenBitNormalisedText(strIn):
    ...
    :return:
    """
    return strIn.upper().replace("À", "A").replace("Á", "A").replace("Â", "A").replace("Ã", "A").replace("Ä",
                                                                                                         "AE").replace(
        "Å", "AA").replace("Æ", "AE").replace("Ç", "C").replace("È", "E").replace("É", "E").replace("Ê", "E").replace(
        "Ë", "E").replace("Ì", "I").replace("Í", "I").replace("Î", "I").replace("Ï", "I").replace("Ñ", "N").replace("Ò",
                                                                                                                    "O").replace(
        "Ó", "O").replace("Ô", "O").replace("Õ", "O").replace("Ö", "OE").replace("Ø", "OE").replace("Ù", "U").replace(
        "Ú", "U").replace("Û", "U").replace("Ü", "UE")
