        ...
        if not ngram.isupper() and not NEWLINE in ngram and count % div == 0: # and ngram.count(" ") < 3:
            ...
            for s in splits:
                ...
                if NUMERIC in ngram:
                    ...
                    print (".")
                    liWeb = GetAcronymsFromWeb.ngramsWebDump(strURL, 1, 10)
                ...
                liCorpus = GetSynonymsFromNgrams.findEmbeddings(leftString, acronym, rightString, ngramstat, index, minWinSize, minfreq, maxcount, minNumberTokens, maxNumberTokens, False)
                ...
                for item in liCorpus:
                    ...
                    if re.search("^[\ \-A-Za-z0-9" + dia +"]*$", exp) != None and acronym.lower() != exp.lower()[0:len(acronym.lower())]:
                        if exp != oldExp:
                            scoreCorpus = 0
                            scoreCorpus = RateAcronymResolutions.GetAcronymScore(acronym, exp, morphemes)
                            if scoreCorpus > 0:
                                result = str(round(scoreCorpus * math.log10(f),2)) + " " + exp  + " " +  str(round(scoreCorpus, 2)) + " " +  str(f) + " " + "\t" + ngram
                ...
                for item in liWeb:
                    ...
                    if re.search("^[\ \-A-Za-z0-9" + dia +"]*$", exp) != None and acronym.lower() != exp.lower()[0:len(acronym.lower())]:
                        if exp != oldExp:
                            scoreWeb = 0
                            scoreWeb = RateAcronymResolutions.GetAcronymScore(acronym, exp, morphemes)
                            if scoreWeb > 0:
                                result = str(round(scoreWeb * math.log10(f),2)) + " " + exp  + " " +  str(round(scoreWeb, 2)) + " " +  str(f) + " " + "\t" + ngram
